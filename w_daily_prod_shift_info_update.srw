$PBExportHeader$w_daily_prod_shift_info_update.srw
$PBExportComments$<Child> shift info update window inherited from w_child
forward
global type w_daily_prod_shift_info_update from w_child
end type
type cb_ok from u_cb within w_daily_prod_shift_info_update
end type
type cb_cancel from u_cb within w_daily_prod_shift_info_update
end type
type st_1 from statictext within w_daily_prod_shift_info_update
end type
type st_3 from statictext within w_daily_prod_shift_info_update
end type
type st_4 from statictext within w_daily_prod_shift_info_update
end type
type em_end from u_em within w_daily_prod_shift_info_update
end type
type cb_save from u_cb within w_daily_prod_shift_info_update
end type
type em_start from u_em within w_daily_prod_shift_info_update
end type
type cb_time from u_cb within w_daily_prod_shift_info_update
end type
type tab_shift from tab within w_daily_prod_shift_info_update
end type
...
type cb_addcoil from u_cb within tabpage_details
end type
type cb_delcoil from u_cb within tabpage_details
end type
...
type dw_1 from u_dw within w_daily_prod_shift_info_update
end type
type tabpage_details from userobject within tab_shift
gb_coil gb_coil
gb_job gb_job
...
string Title="Update Shift Information"
long BackColor=80269524
boolean MinBox=false
boolean MaxBox=false
boolean Resizable=false
event type integer ue_save ( )
event ue_add_coil ( long al_coil,  long al_job )
cb_ok cb_ok
cb_cancel cb_cancel
st_1 st_1
st_3 st_3
...
global w_daily_prod_shift_info_update w_daily_prod_shift_info_update
...
Long ll_bw, ll_ew, ll_coil, ll_netwt_balance
...
UPDATE SHIFT 
Set start_time = :ldt_s, end_time = :ldt_e, note = :tab_shift.tabpage_comments.mle_note.Text
WHERE shift_num = :istr_q.shift_id
USING SQLCA;
...
tab_shift.tabpage_details.dw_list.ResetUpdate()

RETURN 1
end event

event ue_add_coil;Long ll_wt_balance, ll_newcoil, ll_rc, ll_i, ll_coil
Int li_status

ll_rc = tab_shift.tabpage_details.dw_coil.RowCount()
IF ll_rc > 0 THEN
	FOR ll_i = 1 TO ll_rc
		ll_coil = tab_shift.tabpage_details.dw_coil.GetItemNumber(ll_i, "coil_abc_num", Primary!, FALSE)
		IF ll_coil = al_coil THEN
			MessageBox("Warning","Coil had already been assigned to this shift", StopSign!)
			Return 
		END IF
	NEXT
END IF

SELECT coil_status, net_wt_balance
INTO :li_status, :ll_wt_balance
FROM coil
WHERE coil_abc_num = :al_coil
USING SQLCA;

ll_newcoil = tab_shift.tabpage_details.dw_coil.InsertRow(0)
tab_shift.tabpage_details.dw_coil.SetItem(ll_newcoil, "shift_num", istr_q.shift_id)
tab_shift.tabpage_details.dw_coil.SetItem(ll_newcoil, "ab_job_num", al_job)
tab_shift.tabpage_details.dw_coil.SetItem(ll_newcoil, "coil_abc_num", al_coil)
tab_shift.tabpage_details.dw_coil.SetItem(ll_newcoil, "coil_begin_status", li_status)
tab_shift.tabpage_details.dw_coil.SetItem(ll_newcoil, "coil_begin_wt", ll_wt_balance)
...
on w_daily_prod_shift_info_update.create
int iCurrent
call super::create
...
this.Control[iCurrent+9]=this.cb_time
this.Control[iCurrent+10]=this.tab_shift
this.Control[iCurrent+11]=this.dw_1
end on

on w_daily_prod_shift_info_update.destroy
call super::destroy
destroy(this.cb_ok)
...
tab_shift.tabpage_comments.mle_note.Text = ls_note
tab_shift.tabpage_details.dw_coil.Retrieve(istr_q.shift_id)
tab_shift.tabpage_details.dw_list.Retrieve(istr_q.shift_id)
tab_shift.tabpage_dt.dw_dt.Retrieve(istr_q.shift_id)
...
event type integer ue_add_coil ( long al_coil,  long al_job )
int X=15
int Y=90
int Width=2937
int Height=1226
...
int TabOrder=11
boolean BringToTop=true
string DataObject="d_daily_prod_coil_info"
end type
...
OpenWithParm(w_daily_prod_coil_update, istr_q)
lstr_1 = Message.PowerObjectParm
IF lstr_1.coil_num > 0 THEN
	Parent.Event ue_add_coil(lstr_1.coil_num, lstr_1.job_num)
END IF
end event
...
event clicked;IF tab_shift.tabpage_details.dw_coil.RowCount() <= 0 THEN RETURN -1


IF messagebox("Warning", "Do you want to remove this coil?",&
            Question!, OkCancel!,1)<>1 then
 		 Return 0
ELSE	
	tab_shift.tabpage_details.dw_coil.DeleteRow(0)
END IF
...
int TabOrder=11
boolean BringToTop=true
string DataObject="d_daily_prod_new_dt_info_update"
end type
...
SetItem(ll_row, "shift_num", istr_q.shift_id)
