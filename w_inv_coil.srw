...
this.Control[iCurrent+2]=this.cb_qr_manual
this.Control[iCurrent+3]=this.cb_qr_data
this.Control[iCurrent+4]=this.cbx_transfer_ready
this.Control[iCurrent+5]=this.cb_change_status
this.Control[iCurrent+6]=this.rb_all
this.Control[iCurrent+7]=this.rb_default
this.Control[iCurrent+8]=this.dw_coil_list
...
this.Control[iCurrent+14]=this.cb_sendedi
this.Control[iCurrent+15]=this.cb_sort
this.Control[iCurrent+16]=this.dw_rejected
this.Control[iCurrent+17]=this.ddlb_customer
this.Control[iCurrent+18]=this.cb_modify
this.Control[iCurrent+19]=this.cb_reference
this.Control[iCurrent+20]=this.st_1
this.Control[iCurrent+21]=this.st_cond
this.Control[iCurrent+22]=this.cb_search
this.Control[iCurrent+23]=this.cb_reset
...
this.Control[iCurrent+33]=this.dw_editor
...
this.Control[iCurrent+41]=this.dw_coil_history
this.Control[iCurrent+42]=this.cb_import
this.Control[iCurrent+43]=this.cb_archived_bol
this.Control[iCurrent+44]=this.cb_barcode
this.Control[iCurrent+45]=this.gb_2
this.Control[iCurrent+46]=this.r_1
end on
...
is_select = "SELECT  ~~~"COIL~~~".~~~"COIL_ABC_NUM~~~" , ~~~"COIL~~~".~~~"COIL_ORG_NUM~~~" ,~~~"COIL~~~".~~~"ICRA~~~" ,~~~"COIL~~~".~~~"COIL_MID_NUM~~~" ,~~~"COIL~~~".~~~"COIL_STATUS~~~" ,~~~"COIL~~~".~~~"COIL_ALLOY2~~~" ,           ~~~"COIL~~~".~~~"COIL_TEMPER~~~" ,~~~"COIL~~~".~~~"COIL_WIDTH~~~" ,~~~"COIL~~~".~~~"LOT_NUM~~~" , ~~~"COIL~~~".~~~"COIL_GAUGE~~~" ,~~~"COIL~~~".~~~"DATE_RECEIVED~~~" , ~~~"COIL~~~".~~~"COIL_ENTRY_DATE~~~" ,~~~"COIL~~~".~~~"NET_WT~~~" ,~~~"COIL~~~".~~~"PIECES_PER_CASE~~~" , ~~~"COIL~~~".~~~"COIL_LOCATION~~~" ,~~~"COIL~~~".~~~"NET_WT_BALANCE~~~" , ~~~"COIL~~~".~~~"COIL_NOTES~~~" , ~~~"COIL~~~".~~~"COIL_LINE_NUM~~~" ,~~~"COIL~~~".~~~"COIL_EDI856_STATUS~~~" ,~~~"COIL~~~".~~~"CUSTOMER_ID~~~", ~~~"COIL~~~".~~~"COIL_FROM_CUST_ID~~~", f_get_coil_duration(coil.coil_abc_num), f_get_rejected_date(coil.coil_abc_num), f_get_onhold_date(coil.coil_abc_num), f_get_rebanded_date(coil.coil_abc_num),~~~"COIL~~~".~~~"ABCO_COIL_NET_WT~~~",~~~"COIL~~~".~~~"CONSUMED_COIL_NUM~~~",~~~"COIL~~~".~~~"MATERIAL_NUM~~~",~~~"COIL~~~".~~~"CASH_DATE~~~",~~~"COIL~~~".~~~"COIL_ABC_NUM_PREVIOUS~~~",~~~"COIL~~~".~~~"CUSTOMER_ID_PREVIOUS~~~",~~~"COIL~~~".~~~"COIL_ABC_NUM_NEW~~~",~~~"COIL~~~".~~~"CUSTOMER_ID_NEW~~~", dbo.f_get_coil_cert_recevd(coil.coil_org_num)  FROM ~~~"COIL~~~"    "  
...
CHOOSE CASE ls_text 
	...
	CASE ELSE		
		...
		if ii_showall = 1 then
			...
			is_where = "  WHERE ( ~~~"COIL~~~".~~~"CUSTOMER_ID~~~" = :Customer_id and  ~~~"COIL~~~".~~~"COIL_STATUS~~~"  NOT IN (0, 10, 13) )"
		end if
		ll_row = dw_customer.RowCount()
		//dw_coil_list.Object.item_t.visible = false		//hide the total coil number
		FOR ll_i = 1 TO ll_row
