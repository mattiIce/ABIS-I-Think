$PBExportHeader$w_scale_skid.srw
$PBExportComments$<Child> skid weight information retrieving from scale
forward
global type w_scale_skid from w_child
end type
type cb_print from u_cb within w_scale_skid
end type
type st_msg from statictext within w_scale_skid
end type
type st_5 from statictext within w_scale_skid
end type
type em_net from u_em within w_scale_skid
end type
type dw_report from u_dw within w_scale_skid
end type
type sle_input from u_sle within w_scale_skid
end type
type em_id from u_em within w_scale_skid
end type
type st_4 from statictext within w_scale_skid
end type
type p_1 from picture within w_scale_skid
end type
type pb_scale from u_pb within w_scale_skid
end type
type st_3 from statictext within w_scale_skid
end type
type st_2 from statictext within w_scale_skid
end type
type st_1 from statictext within w_scale_skid
end type
type em_tare from u_em within w_scale_skid
end type
type em_gross from u_em within w_scale_skid
end type
type cb_save from u_cb within w_scale_skid
end type
type cb_new from u_cb within w_scale_skid
end type
type cb_exit from u_cb within w_scale_skid
end type
end forward
...
global w_scale_skid w_scale_skid
...
event pfc_save;Long ll_skid, ll_other_items_wt, ll_item_wt, ll_no_wt_item_id, ll_db_skid_wt, ll_current_item_wt, ll_current_skid_wt, ll_tare, ll_current_item_wt_by_pc
Int li_row, li_i, li_no_wt_item
Long ll_skid_pc, ll_other_items_pc, ll_item_pc, ll_no_wt_item_pc
DataStore lds_skid
...
IF il_tare > 0 THEN 
	...
	FOR li_i = 1 TO li_row
		...
		ll_skid_pc = lds_skid.GetItemNumber(li_i, "sheet_skid_skid_pieces")
		ll_item_pc = lds_skid.GetItemNumber(li_i, "production_sheet_item_prod_item_pieces")
		IF IsNULL(ll_item_pc) OR (ll_item_pc = 0) THEN
			this.Event ue_display_message("Warning : Missing pieces count for skid items. Please verify.")
			DESTROY lds_skid
			RETURN -3
		END IF
		...
		ll_other_items_pc = ll_other_items_pc + ll_item_pc
	NEXT
	...
	ll_no_wt_item_pc = ll_skid_pc - ll_other_items_pc
	...
	IF ll_current_item_wt <= 0 THEN
		...
		RETURN -6
	END IF
	IF ll_no_wt_item_pc <= 0 THEN
		this.Event ue_display_message("Warning : Invalid skid item pieces.")
		DESTROY lds_skid
		RETURN -6
	END IF
	...
	FOR li_i = 1 TO li_row
		...
		ll_item_wt = lds_skid.GetItemNumber(li_i, "production_sheet_item_prod_item_net_wt")
		IF IsNULL(ll_item_wt) OR (ll_item_wt = 0) THEN	
			IF ll_item_pc = ll_no_wt_item_pc THEN
				ll_current_item_wt_by_pc = ll_current_item_wt
			ELSE
				ll_no_wt_item_pc = ll_no_wt_item_pc - ll_item_pc
				ll_current_item_wt_by_pc = ll_current_item_wt * (ll_item_pc / ll_no_wt_item_pc)
				ll_current_item_wt = ll_current_item_wt - ll_current_item_wt_by_pc 
				ll_no_wt_item_pc = ll_no_wt_item_pc - ll_item_pc
			END IF
			UPDATE production_sheet_item
			SET prod_item_net_wt = :ll_current_item_wt_by_pc
