$PBExportHeader$w_security_usermgmt.srw
$PBExportComments$Manage users and groups
forward
global type w_security_usermgmt from w_sheet
end type
type tv_users from u_tv within w_security_usermgmt
end type
type dw_users from u_dw within w_security_usermgmt
end type
type tv_groups from u_tv within w_security_usermgmt
end type
type dw_groups from u_dw within w_security_usermgmt
end type
type st_1 from u_st within w_security_usermgmt
end type
type st_2 from u_st within w_security_usermgmt
end type
end forward
...
string menuname = "m_security_user_mgmt"
...
global w_security_usermgmt w_security_usermgmt
...
string		is_group
window	iw_frame
m_security_user_mgmt	im_popmenu
end variables
...
s_security_user_data lstr_data
n_ds l_ds
long ll_row
...
openwithparm(w_security_edituser,lstr_data)
...
if ii_usertype < 2 then // ok, were deleting a user or a group so we have to worry about removing all instances of the item from all tables in the database
	...
	if ii_usertype = 0 then // user
		...
		l_ds_delete.dataobject = 'd_security_grouplookup'
	else // group
		tv_groups.of_DeleteItem(il_currhandle)
		l_ds_delete.dataobject = 'd_security_user_lookup_by_group'
	end if
	...
	if li_cnt > 0 then 
		...
		If li_rc = 1 then // then delete all entries in the info table for this user
			l_ds_delete.dataobject = 'd_security_info_user_delete'
			l_ds_delete.settransobject(SQLCA)
			li_cnt = l_ds_delete.retrieve(lstr_data.name)
			if li_cnt > 0 then 
				l_ds_delete.rowsmove(1,li_cnt,primary!,l_ds_delete,1,delete!)
...
on w_security_usermgmt.create
int iCurrent
call super::create
if this.MenuName = "m_security_user_mgmt" then this.MenuID = create m_security_user_mgmt
...
on w_security_usermgmt.destroy
call super::destroy
...
//
//	Object Name:  w_security_user_mgmt
//
...
li_rc = tv_users.of_SetDatasource(1, "d_security_users_update", SQLCA, "description", "", False, li_rc,li_rc)
li_rc = tv_users.AddPicture("group!")
li_rc = tv_users.of_SetDatasource(2, "d_security_grouplookup", SQLCA, "description", ":parent.1.name", False, li_rc,li_rc)

li_rc = tv_groups.AddPicture("group!")
li_rc = tv_groups.of_SetDatasource(1, "d_security_users_update", SQLCA, "description", "", False,  li_rc,li_rc)
li_rc = tv_groups.AddPicture("custom076!")
li_rc = tv_groups.of_SetDatasource(2, "d_security_user_lookup_by_group", SQLCA, "description", ":parent.1.name", False,  li_rc,li_rc)


//is_savetitle = this.title
...
integer height = 1046
integer taborder = 30
string dragicon = "Q:\silverdome\ArtGlry\USER.ico"
...
string statepicturename[] = {"he Clipboard"}
long statepicturemaskcolor = 553648127
end type
