$PBExportHeader$w_downtime_detail_partid.srw
$PBExportComments$<Child> display downtime details based on enduser's part number
forward
global type w_downtime_detail_partid from w_child
end type
type dw_report from u_dw within w_downtime_detail_partid
end type
type cb_close from u_cb within w_downtime_detail_partid
end type
type cb_print from u_cb within w_downtime_detail_partid
end type
type cb_email from u_cb within w_downtime_detail_partid
end type
type cb_save from u_cb within w_downtime_detail_partid
end type
type dw_detail from u_dw within w_downtime_detail_partid
end type
type st_1 from statictext within w_downtime_detail_partid
end type
type cbx_rotate from u_cbx within w_downtime_detail_partid
end type
end forward
...
string Title="Downtime details based  on enduser part number"
string Tag="detail downtime information based on enduser part number"
...
global w_downtime_detail_partid w_downtime_detail_partid
...
on w_downtime_detail_partid.create
int iCurrent
call super::create
...
on w_downtime_detail_partid.destroy
call super::destroy
...
dw_report.Retrieve(is_info.part_num)
...
WHERE ( DT_INSTANCE_DETAIL.INSTANCE_NUM = DT_INSTANCE.INSTANCE_NUM ) and  
       ...
       ( AB_JOB.AB_JOB_NUM = DT_INSTANCE.AB_JOB_NUM ) and  
       ( ORDER_ITEM.ORDER_ITEM_NUM = AB_JOB.ORDER_ITEM_NUM ) and  
       ( ORDER_ITEM.ORDER_ABC_NUM = AB_JOB.ORDER_ABC_NUM ) and  
       ( ( ORDER_ITEM.ENDUSER_PART_NUM = :is_info.part_num ) );   

dw_report.Object.gr_1.Title = "Downtime by part " + String(is_info.part_num) + " - " + String(lr_minutes, "########") +" mins"
...
int Height=2202
int TabOrder=10
string Tag="Detail downtime information of  a enduser part"
string DataObject="d_report_downtime_details_per_partid"
