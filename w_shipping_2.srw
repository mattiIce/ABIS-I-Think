$PBExportHeader$w_shipping_2.srw
$PBExportComments$<Sheet>w_shipping version 2 inherited from pfemain\w_sheet
forward
global type w_shipping_2 from w_sheet
end type
type dw_shipment_lock_users from datawindow within w_shipping_2
end type
type cb_get_coils from commandbutton within w_shipping_2
end type
type dw_error_message from datawindow within w_shipping_2
end type
type cb_1 from commandbutton within w_shipping_2
end type
type st_10 from statictext within w_shipping_2
end type
type cb_browse from commandbutton within w_shipping_2
end type
type cb_save from commandbutton within w_shipping_2
end type
type cb_edi from commandbutton within w_shipping_2
end type
type cb_new from commandbutton within w_shipping_2
end type
type cb_close from commandbutton within w_shipping_2
end type
type tab_shiped_items from tab within w_shipping_2
end type
...
type st_19 from u_st within w_shipping_2
end type
type st_shipmentwt from u_st within w_shipping_2
end type
type cb_print from commandbutton within w_shipping_2
end type
type dw_shipment_detail from u_dw within w_shipping_2
end type
type st_2 from u_st within w_shipping_2
end type
type st_shipmentitem from u_st within w_shipping_2
end type
type st_12 from statictext within w_shipping_2
end type
type st_ti1 from statictext within w_shipping_2
end type
type st_13 from statictext within w_shipping_2
end type
type st_scti1 from statictext within w_shipping_2
end type
type st_14 from statictext within w_shipping_2
end type
type st_rti1 from statictext within w_shipping_2
end type
type st_15 from statictext within w_shipping_2
end type
type st_whti1 from statictext within w_shipping_2
end type
type cb_revise from commandbutton within w_shipping_2
end type
type st_7 from statictext within w_shipping_2
end type
type st_bol from statictext within w_shipping_2
end type
type st_8 from statictext within w_shipping_2
end type
type ddlb_packinglist from u_ddlb within w_shipping_2
end type
type st_multistop from statictext within w_shipping_2
end type
type cb_newstop from commandbutton within w_shipping_2
end type
type gb_bol from groupbox within w_shipping_2
end type
end forward
...
dw_shipment_lock_users dw_shipment_lock_users
cb_get_coils cb_get_coils
...
global w_shipping_2 w_shipping_2
...
public function integer wf_validate_coil_status ()
public function boolean wf_is_novelis (long al_customer_id)
public function integer wf_get_shipment_table_locks ()
end prototypes
...
IF li_totalrow > 0 THEN 
	//Alex Gerlants. 03/14/2019. Begin
	Boolean	lb_is_novelis
	
	lb_is_novelis = wf_is_novelis(il_current_customer)
	
	If lb_is_novelis Then //Customer is Novelis
		//Do not update coil_status to 10 (Gone) for Novelis here.
		//For Novelis, it will be updated in a new database function edi_novelis_rejcoil_856_X12 
		//because now for Novelis, we send ASN for scrap/rejected/return coils.
	Else //Customer is NOT Novelis
	//Alex Gerlants. 03/14/2019. End	
	...
	End If//Alex Gerlants. 03/14/2019
END IF
...
Boolean					lb_is_novelis //Alex Gerlants. Print Cert Label
...
//Alex Gerlants. Print Cert Label. Begin
lb_is_novelis = wf_is_novelis(il_current_customer)

If lb_is_novelis Or il_current_customer = 2784 Then //Alex Gerlants. 06/13/2019. Added " Or il_current_customer = 2784" for Arconic-TN
	OpenWithParm(w_asn_reminder, il_current_shippment)
End If
//Alex Gerlants. Print Cert Label. End
end subroutine

public function integer wf_validate_coil_status ();//Alex Gerlants. novelis testing. Begin
/*
Function:	wf_validate_coil_status
Returns:		integer	 1 if ok
							-1 if error
Arguments;	none							
*/

Integer	li_rtn = 1, li_coil_status, li_coil_status_prev
Long		ll_rows, ll_row, ll_coil_abc_num
Boolean	lb_different_coil_status = False
String	ls_coil_string, ls_coil_status_desc

ll_rows = tab_shiped_items.tabpage_rej_coil_ed.dw_rej_coil_item.RowCount()

For ll_row = 1 To ll_rows
	ll_coil_abc_num = tab_shiped_items.tabpage_rej_coil_ed.dw_rej_coil_item.Object.coil_abc_num[ll_row]
	
	If ll_coil_abc_num > 0 Then
		select	coil.coil_status, dbo.coil_status.coil_status_desc
		into		:li_coil_status, :ls_coil_status_desc
		from		coil
					join dbo.coil_status on dbo.coil_status.coil_status_code = coil.coil_status
		where		coil.coil_abc_num = :ll_coil_abc_num
		using		sqlca;
		
		If sqlca.sqlcode = 0 Then //OK
			ls_coil_string = ls_coil_string + "   " + String(ll_coil_abc_num) + ": " + ls_coil_status_desc + "~n~r"
		
			If ll_row > 1 Then
				If li_coil_status <> li_coil_status_prev Then
					lb_different_coil_status = True
				End If
			End If
		Else //Error: coil not found or DB error
			//
		End If
		
		li_coil_status_prev = li_coil_status
	End If
Next

If lb_different_coil_status Then
	li_rtn = -1
	
	MessageBox("Not allowed to ship" , 	"The following coil ABC numbers have different status:~n~r" + ls_coil_string + &
													"~n~r~n~rNot allowed to ship." + & 
													"~n~r~Each coil status must be on a separate stop." + & 
													"~n~rPlease correct. ~n~rExiting...", StopSign!)
End If
...
public function boolean wf_is_novelis (long al_customer_id);//Alex Gerlants. Form_863 Cert Label. Begin
/*
Function:	wf_is_novelis
Returns:		boolean	True  if al_customer_id is Novelis
							False if al_customer_id is NOT Novelis
Arguments:	value		long	al_customer_id
*/
Boolean	lb_is_novelis = False
Integer	li_count
...
from		customer
where		customer_id = :al_customer_id
and		lower(ltrim(rtrim(customer_short_name))) like '%novelis%'
using		sqlca;
...
Return lb_is_novelis
//Alex Gerlants. Form_863 Cert Label. End
end function

public function integer wf_get_shipment_table_locks ();//Alex Gerlants. Shipment_Table_Lock. Begin
/*
Function:	wf_get_shipment_table_locks
Returns:		integer	 1 if shipment table is locked
							 0 if shipment table is not locked
							-1 if DB error
Arguments:	none							
*/
Integer		li_rtn = 0, li_rows, li_row, li_answer, li_sid
String		ls_object_type, ls_object_name, ls_osuser, ls_err_string
String		ls_shipment_lock_user, ls_current_user, ls_status
DataWindow	ldw_oracle_locks
Transaction	ltr

/*
Create a transaction with user DBO because this user has been granted access to tables: 
v_$lock, v_$locked_object, dba_objects, v_$session, v_$sql

This is done on the server: sqlplus connect as sysdba

GRANT SELECT ON v_$lock TO DBO;
GRANT SELECT ON v_$locked_object to DBO;
GRANT SELECT ON dba_objects to DBO;
GRANT SELECT ON v_$session to DBO;
GRANT SELECT ON v_$sql to DBO;
*/

ls_current_user = sqlca.logid

If IsValid(ltr) Then Destroy ltr
...
If ltr.sqlcode < 0 Then Return -1
...
If ltr.sqlcode < 0 Then //DB error
	li_rtn = -1
	
	MessageBox("DB error",	"Database error in wf_get_shipment_table_locks for w_shipment2 while retrieving current sid.~n~r~n~r" + &
									"Error:~n~r" + ltr.sqlerrtext)
End If

If li_rtn <> -1 Then
	select	nvl2(lockwait, 'BLOCKED', status)
	into		:ls_status
	from		v$session s
	where		sid = :li_sid
	using		ltr;
	...
	If ltr.sqlcode < 0 Then //DB error
		...
		MessageBox("DB error",	"Database error in wf_get_shipment_table_locks for w_shipment2 while retrieving status for current sid " + + String(li_sid) + ".~n~r~n~r" + &
										"Error:~n~r" + ltr.sqlerrtext)
	End If
End If
	
If li_rtn <> -1 Then
	If ls_status = 'BLOCKED' Then //Current session is blocked
	
		dw_shipment_lock_users.SetTransObject(ltr)
		li_rows = dw_shipment_lock_users.Retrieve()
		
		If li_rows > 0 Then
			For li_row = 1 To li_rows
				ls_shipment_lock_user = dw_shipment_lock_users.Object.shipment_lock_user[li_row]
				
				If ls_shipment_lock_user <> ls_current_user Then
					ls_err_string = ls_err_string + ls_shipment_lock_user + ", "
				End If
			Next
			...
			If ls_err_string <> "" Then
				...
				li_answer = MessageBox("Shipment table is locked", "Please ask " + ls_err_string + " to exit abis.~n~r" + &
								"~n~rClick on 'Yes' to continue saving." + &
								"~n~rClick on 'No' to exit without saving.", &
								Question!, YesNo!, 2) //Make button "No" selected as default
	...
	End If //If ls_status = 'BLOCKED' Then
End If //If li_rtn <> -1 Then

Disconnect using ltr;
If IsValid(ltr) Then Destroy ltr

Return li_rtn
//Alex Gerlants. Shipment_Table_Lock. End
end function

on w_shipping_2.create
int iCurrent
call super::create
if this.MenuName = "m_shipment_shipping" then this.MenuID = create m_shipment_shipping
this.dw_shipment_lock_users=create dw_shipment_lock_users
this.cb_get_coils=create cb_get_coils
...
this.Control[iCurrent+1]=this.dw_shipment_lock_users
this.Control[iCurrent+2]=this.cb_get_coils
this.Control[iCurrent+3]=this.dw_error_message
this.Control[iCurrent+4]=this.cb_1
this.Control[iCurrent+5]=this.st_10
this.Control[iCurrent+6]=this.cb_browse
this.Control[iCurrent+7]=this.cb_save
this.Control[iCurrent+8]=this.cb_edi
this.Control[iCurrent+9]=this.cb_new
this.Control[iCurrent+10]=this.cb_close
this.Control[iCurrent+11]=this.tab_shiped_items
this.Control[iCurrent+12]=this.st_19
this.Control[iCurrent+13]=this.st_shipmentwt
this.Control[iCurrent+14]=this.cb_print
this.Control[iCurrent+15]=this.dw_shipment_detail
this.Control[iCurrent+16]=this.st_2
this.Control[iCurrent+17]=this.st_shipmentitem
this.Control[iCurrent+18]=this.st_12
this.Control[iCurrent+19]=this.st_ti1
this.Control[iCurrent+20]=this.st_13
this.Control[iCurrent+21]=this.st_scti1
this.Control[iCurrent+22]=this.st_14
this.Control[iCurrent+23]=this.st_rti1
this.Control[iCurrent+24]=this.st_15
this.Control[iCurrent+25]=this.st_whti1
this.Control[iCurrent+26]=this.cb_revise
this.Control[iCurrent+27]=this.st_7
this.Control[iCurrent+28]=this.st_bol
this.Control[iCurrent+29]=this.st_8
this.Control[iCurrent+30]=this.ddlb_packinglist
this.Control[iCurrent+31]=this.st_multistop
this.Control[iCurrent+32]=this.cb_newstop
this.Control[iCurrent+33]=this.gb_bol
end on

on w_shipping_2.destroy
call super::destroy
if IsValid(MenuID) then destroy(MenuID)
destroy(this.dw_shipment_lock_users)
destroy(this.cb_get_coils)
...
dw_shipment_lock_users.Visible = False //Alex Gerlants. Shipment_Table_Lock
end event
...
String	ls_ship_date, ls_rtn //Alex Gerlants. 01/09/2019. Alex_Ship_Date_Warning
Integer	li_pos, li_answer //Alex Gerlants. 01/09/2019. Alex_Ship_Date_Warning
Long		ll_daysafter //Alex Gerlants. 01/09/2019. Alex_Ship_Date_Warning

Integer	li_rtn //Alex Gerlants. novelis testing

Boolean	lb_is_novelis //Alex Gerlants. Print Cert Label

//Alex Gerlants. Shipment_Table_Lock. Begin
If wf_get_shipment_table_locks() = 1 Then //Shipment table is locked by a user or users other than the current user
	Return 0 //Message is in wf_get_shipment_table_locks()
End If
//Alex Gerlants. Shipment_Table_Lock. End
...
If tab_shiped_items.tabpage_rej_coil_ed.dw_rej_coil_item.RowCount() > 0 Then
	li_rtn = wf_validate_coil_status()
	
	If li_rtn = -1 Then //Reject/Return/Scrap coils selected for this shipment have different status... Not allowed to ship
		Return -1
	End If
End If
//Alex Gerlants. novelis testing. End
...
//Alex Gerlants. 01/09/2019. Alex_Ship_Date_Warning. Begin
ls_ship_date = String(dw_shipment_detail.Object.shipment_actualed_date_time[ll_row])

If Not IsNull(ls_ship_date) Then
	li_pos = Pos(ls_ship_date, " ", 1)
	
	If li_pos > 0 Then
		ls_ship_date = Left(ls_ship_date, li_pos - 1)
		ll_daysafter = DaysAfter(Today(), Date(ls_ship_date))
		
		If ll_daysafter < 0 Then
			SetFocus(dw_shipment_detail)
			dw_shipment_detail.ScrollToRow(ll_row)
			dw_shipment_detail.SetRow(ll_row)
			dw_shipment_detail.SetColumn("shipment_actualed_date_time")
			ls_rtn = dw_shipment_detail.Modify("shipment_actualed_date_time.Color = '255'") //Red color
			
			li_answer = MessageBox("Warning", "Ship Date '" + ls_ship_date + "' is prior to Current Date '" + String(Today()) + "'" + &
											"~n~rAre you sure?", Question!, YesNo!, 2)
			
			If li_answer = 1 Then
				ls_rtn = dw_shipment_detail.Modify("shipment_actualed_date_time.Color = '0'") //Turn it back to black color
			Else //li_answer = 2
				Return 1
			End If
		Else //ll_daysafter >= 0...OK
			ls_rtn = dw_shipment_detail.Modify("shipment_actualed_date_time.Color = '0'") //Black color
		End If
	Else //Should not happen
		MessageBox("Error", 	"Error in pfc_save event for w_shipping_2 while validating Ship Date." + &
									"~n~rShip Date '" + ls_ship_date + "' is no a date.~n~rPlease call Support", StopSign!)
		Return 1
	End If
End If
//Alex Gerlants. 01/09/2019. Alex_Ship_Date_Warning. End
...
CHOOSE CASE Message.DoubleParm
	CASE 1
		...
		if isnull(ld_ship_date) then
			...
			If lb_is_novelis Then
				//Do nothing for Novelis, Warning message for Novelis is in ue_shipment_sent event.
			Else
			//Alex Gerlants. Print Cert Label. End
			...
			End If //Alex Gerlants. Print Cert Label
		end if
...
integer taborder = 80
string title = "none"
string dataobject = "d_shipment_lock_users"
...
event clicked;DataStore	lds_coils_2check
Long			ll_rows, ll_row, ll_ab_job_num, ll_coil_abc_num, ll_sheet_skid_num
String		ls_sheet_skid_display_num, ls_coil_org_num
String		ls_line

lds_coils_2check = Create DataStore
lds_coils_2check.DataObject = "d_coils_2check"
lds_coils_2check.SetTransObject(sqlca)

ll_rows = lds_coils_2check.Retrieve(il_current_shippment)

If ll_rows > 0 Then
	ls_line = "Skid #~tAB Job Num~tCoil ABC Num~tCoil Org Num~n~r"
	
	If ll_rows = 1 Then
		ll_sheet_skid_num = lds_coils_2check.Object.sheet_skid_num[1]
		ll_ab_job_num = lds_coils_2check.Object.ab_job_num[1]
		ll_coil_abc_num = lds_coils_2check.Object.coil_abc_num[1]
		ls_coil_org_num = lds_coils_2check.Object.coil_org_num[1]
		 
		ls_line = ls_line + String(ll_sheet_skid_num) + "~t" + String(ll_ab_job_num) + "~t~t" + String(ll_coil_abc_num) + "~t~t" + ls_coil_org_num
	Else
		For ll_row = 1 To ll_rows
			ll_sheet_skid_num = lds_coils_2check.Object.sheet_skid_num[ll_row]
			ll_ab_job_num = lds_coils_2check.Object.ab_job_num[ll_row]
			ll_coil_abc_num = lds_coils_2check.Object.coil_abc_num[ll_row]
			ls_coil_org_num = lds_coils_2check.Object.coil_org_num[ll_row]
			
			ls_line = ls_line + String(ll_sheet_skid_num) + "~t" + String(ll_ab_job_num) + "~t~t" + String(ll_coil_abc_num) + "~t~t" + ls_coil_org_num + "~n~r" 
		Next
	End if
	
	MessageBox("Coil(s) for Shipment " + 	String(il_current_shippment), ls_line)
End If
end event
		...
		CHOOSE CASE li_return
			...
			CASE 200
				MessageBox("Info","Customer does NOT require EDI transaction for BOL#" + String(ll_shippment) +"." + &
										"~n~rCustomer or destination customer DUNS number not set up.")
			CASE 300
...
event clicked;//Alex Gerlants. 10/04/2018. Begin
Long	ll_rows
Boolean 	lb_valid
String	ls_title
Window 	lw_sheet
...
If lb_valid Then
	...
	Do
		...
		If lb_valid Then
			...
			If Upper(ls_title) = "ABCO SHIPMENT MANAGEMENT" Then
				lw_sheet.Function Dynamic wf_retrieve() //Re-retrieve shipment list
				lw_sheet.Function Dynamic wf_scroll_2row(il_current_shippment) //Scroll to the shipment row that was highlighted
			End If
		End If
	Loop While lb_valid
End If

If IsValid(w_shipment_list) Then
	ll_rows = w_shipment_list.wf_retrieve()
	
	If ll_rows > 0 Then
		w_shipment_list.wf_scroll_2row(il_current_shippment)
	End If
End If
...
st_ti.Text = String(tab_shiped_items.tabpage_sheet_ed.dw_sheet_item.RowCount()) //Alex Gerlants. Update_Skid_Count_on_Shipment_Screen
