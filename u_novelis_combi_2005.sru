$PBExportHeader$u_novelis_combi_2005.sru
forward
global type u_novelis_combi_2005 from u_report_object
end type
end forward
...
global u_novelis_combi_2005 u_novelis_combi_2005
...
lds_print.DataObject = "d_report_abco_combi_form_novelis"  
...
IF ids_scrap.RowCount() > 0 THEN		
	...
	FOR li_i = 1 TO ids_scrap.RowCount()
		...
		//Added by Victor Huang in 10/05 for tote number handling
		li_tote_num = ids_scrap.GetItemNumber(li_i, "scrap_skid_tote_num", Primary!, FALSE)
		...
		IF isnull(li_tote_num) THEN
			lds_print.SetItem(ll_newrow, "comments_t", ls_alloy)
		ELSE
			lds_print.SetItem(ll_newrow, "comments_t", ls_alloy + "          TOTE#: " + String(li_tote_num) ) 
		END IF
	NEXT
...
Long ll_jobpc, ll_jobnet,  ll_jobtheo, ll_jobtare, ref_order
Long ll_jobnet_kg,  ll_jobtheo_kg, ll_jobtare_kg

Int li_s, li_shipitem_num, li_skidcount
Long ll_skid_num, ll_job_num, ll_net_wt, ll_skidpc, ll_tarewt
...
Boolean 	lb_cash_date_on_bol //Alex Gerlants. 08/18/2018. Add Cash Date 2BOL
String	ls_cash_date, ls_coil_org_num, ls_old_sort, ls_find_string //Alex Gerlants. 08/18/2018. Add Cash Date 2BOL
Long		ll_rows, ll_row, ll_found_row //Alex Gerlants. 08/18/2018. Add Cash Date 2BOL
DataStore	lds_min_cash_date
...
//Alex Gerlants. Add Cash Date 2BOL. Begin
f_get_cash_date_on_bol_4shipmen(al_shipment, sqlca, lb_cash_date_on_bol)

If lb_cash_date_on_bol Then
	lds_print.DataObject = "d_report_abco_combi_form_novelis_cd"
End If
//Alex Gerlants. Add Cash Date 2BOL. End
		...
		FOR li_i = 1 TO li_totaljob
			...
			FOR li_s = 1 TO ids_sheet.RowCount()
				...
				IF istr_job_info[li_i].abco_num = ll_job_num and istr_job_info[li_i].ref_order_num = ref_order and istr_job_info[li_i].ref_order_item_num = ref_item THEN
					...
					ll_jobtare_kg = ll_jobtare_kg + Long(String(ll_tarewt * 0.45356, "#,###,###"))

					il_shippc = il_shippc + ll_skidpc
					il_shiptare = il_shiptare + ll_tarewt					
					il_shiptare_kg = il_shiptare_kg + Long(String(ll_tarewt * 0.45356, "#,###,###"))					
					...
					IF ll_itemrow > 0 THEN
						ll_gross_kg = Long(String(ll_tarewt * 0.45356, "#,###,###"))
						FOR ll_itemi = 1 TO ll_itemrow
							ll_gross_kg = ll_gross_kg + Long( String(lds_sheetitem.GetItemNumber(ll_itemi, "production_sheet_item_prod_item_net_wt")* 0.45356, "#,###,###") )
						NEXT
						...
						lds_print.SetItem(ll_newrow, "ntweight_kg_t", String(lds_sheetitem.GetItemNumber(1, "production_sheet_item_prod_item_net_wt") * 0.45356, "#,###,###"))
						lds_print.SetItem(ll_newrow, "theowt_t", String(lds_sheetitem.GetItemNumber(1, "production_sheet_item_prod_item_theoreti"), "#,###,###"))
						lds_print.SetItem(ll_newrow, "theowt_kg_t", String(lds_sheetitem.GetItemNumber(1, "production_sheet_item_prod_item_theoreti")*0.45356, "#,###,###"))				
						lds_print.SetItem(ll_newrow, "tarewt_t", String(ll_tarewt, "#,###,###"))
						lds_print.SetItem(ll_newrow, "tarewt_kg_t", String(ll_tarewt * 0.45356, "#,###,###"))
						...
						ll_jobnet_kg = ll_jobnet_kg + Long(String(lds_sheetitem.GetItemNumber(1, "production_sheet_item_prod_item_net_wt")* 0.45356, "#,###,###"))
						ll_jobtheo = ll_jobtheo + lds_sheetitem.GetItemNumber(1, "production_sheet_item_prod_item_theoreti")
						ll_jobtheo_kg = ll_jobtheo_kg + Long(String(lds_sheetitem.GetItemNumber(1, "production_sheet_item_prod_item_theoreti")*0.45356, "#,###,###"))
						il_shipnet = il_shipnet + lds_sheetitem.GetItemNumber(1, "production_sheet_item_prod_item_net_wt")
						il_shipnet_kg = il_shipnet_kg + Long(String(lds_sheetitem.GetItemNumber(1, "production_sheet_item_prod_item_net_wt")* 0.45356, "#,###,###"))
						il_shiptheo = il_shiptheo + lds_sheetitem.GetItemNumber(1, "production_sheet_item_prod_item_theoreti")
						il_shiptheo_kg = il_shiptheo_kg + Long(String(lds_sheetitem.GetItemNumber(1, "production_sheet_item_prod_item_theoreti")*0.45356, "#,###,###"))
						...
						IF ll_itemrow > 1 THEN
							FOR ll_itemi = 2 TO ll_itemrow
								...
								lds_print.SetItem(ll_newrow, "ntweight_kg_t", String(lds_sheetitem.GetItemNumber(ll_itemi, "production_sheet_item_prod_item_net_wt") * 0.45356, "#,###,###"))
								...
								lds_print.SetItem(ll_newrow, "theowt_kg_t", String(lds_sheetitem.GetItemNumber(ll_itemi, "production_sheet_item_prod_item_theoreti")*0.45356, "#,###,###"))				
								...
								ll_jobnet_kg = ll_jobnet_kg + Long(String(lds_sheetitem.GetItemNumber(ll_itemi, "production_sheet_item_prod_item_net_wt")* 0.45356, "#,###,###"))
								ll_jobtheo = ll_jobtheo + lds_sheetitem.GetItemNumber(ll_itemi, "production_sheet_item_prod_item_theoreti")
								ll_jobtheo_kg = ll_jobtheo_kg + Long(String(lds_sheetitem.GetItemNumber(ll_itemi, "production_sheet_item_prod_item_theoreti")*0.45356, "#,###,###"))
								il_shipnet = il_shipnet + lds_sheetitem.GetItemNumber(ll_itemi, "production_sheet_item_prod_item_net_wt")
								il_shipnet_kg = il_shipnet_kg + Long(String(lds_sheetitem.GetItemNumber(ll_itemi, "production_sheet_item_prod_item_net_wt")* 0.45356, "#,###,###"))
								il_shiptheo = il_shiptheo + lds_sheetitem.GetItemNumber(ll_itemi, "production_sheet_item_prod_item_theoreti")
								il_shiptheo_kg = il_shiptheo_kg + Long(String(lds_sheetitem.GetItemNumber(ll_itemi, "production_sheet_item_prod_item_theoreti")*0.45356, "#,###,###"))
			...
			//Alex Gerlants. 08/18/2018. Add Cash Date 2BOL. Begin
			If lb_cash_date_on_bol Then
				lds_min_cash_date = Create DataStore
				lds_min_cash_date.DataObject = "d_min_cash_date"
				lds_min_cash_date.SetTransObject(sqlca)
				
				ll_rows = lds_min_cash_date.Retrieve(al_shipment)
				
				For ll_row = 1 To ll_rows
					ls_cash_date = lds_min_cash_date.Object.cash_date[ll_row]
					If ls_cash_date = "//" Then ls_cash_date = "NA"
					ls_coil_org_num = lds_min_cash_date.Object.coil_org_num[ll_row]
					
					ls_find_string = "coil_t = '" + ls_coil_org_num + "'"
					ll_found_row = lds_print.Find(ls_find_string, 1, lds_print.RowCount())
					
					Do While ll_found_row > 0
						lds_print.Object.cash_date[ll_found_row] = "Cash Date:" + ls_cash_date
						ll_found_row = lds_print.Find(ls_find_string, ll_found_row + 1, lds_print.RowCount())
					Loop
				Next
			End If
			//Alex Gerlants. 08/18/2018. Add Cash Date 2BOL. End
...
DateTime ld_date
String ls_carrier, ls_1, ls_2, ls_3, ls_4, ls_city, ls_state, ls_zip, ls_refs, ls_auth_code
Int li_i, li_j
...
CONNECT USING SQLCA;
SELECT BILL_OF_LADING, DES_SH_CUST_ID, CUSTOMER_ID, SHIPMENT_ACTUALED_DATE_TIME, Carrier_id, shipment_reference_codes,shipment_athorization_code
INTO :ll_bill_lading, :ll_des_cust_id, :ll_cust_id, :ld_date, :ll_carrier_id, :ls_refs, :ls_auth_code
FROM SHIPMENT  
WHERE PACKING_LIST = :al_shipment    
USING SQLCA;
...
idw_requestor.SetItem(ll_row, "ref1_t", ls_auth_code)	
...
DateTime ld_date
String ls_carrier, ls_1, ls_2, ls_3, ls_4, ls_city, ls_state, ls_zip, ls_auth_code
String	ls_equipment_type_desc, ls_trailer_num //Alex Gerlants. 07/12/2016
String	ls_vehicle_id, ls_carrier_initial //Alex Gerlants. 07/12/2016

CONNECT USING SQLCA;
SELECT BILL_OF_LADING, DES_SH_CUST_ID, CUSTOMER_ID, SHIPMENT_ACTUALED_DATE_TIME, Carrier_id, shipment_athorization_code
INTO :ll_bill_lading, :ll_des_cust_id, :ll_cust_id, :ld_date, :ll_carrier_id, :ls_auth_code
FROM SHIPMENT  
WHERE PACKING_LIST = :al_shipment    
USING SQLCA;
...
ls_modstring = "date_t.Text = ~"" + String(ld_date, "mm/dd/yyyy hh:mm") + "~""	
ads_print.Modify(ls_modstring) 
//ls_modstring = "ref1_t.Text = ~"" + String(ld_date, "mm/dd/yyyy hh:mm") + "~""	
//ads_print.Modify(ls_modstring) 
...
on u_novelis_combi_2005.create
call super::create
end on

on u_novelis_combi_2005.destroy
call super::destroy
end on
...
IF li_totalrow > 0 THEN //retrieve order information
	...
	FOR li_int = 1 TO li_totalrow
		...
		String ls_alloy, ls_temper, ls_sheet_type, ls_cust_name
		Long ll_net_wt	
