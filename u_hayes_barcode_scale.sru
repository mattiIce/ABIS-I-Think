$PBExportHeader$u_hayes_barcode_scale.sru
$PBExportComments$print hayes barcode from scale
forward
global type u_hayes_barcode_scale from u_report_object
end type
end forward

global type u_hayes_barcode_scale from u_report_object
event type integer ue_print_barcode ( long al_shipment,  long al_skid_num )
event type integer ue_print_barcode_master ( long al_shipment )
end type
global u_hayes_barcode_scale u_hayes_barcode_scale
...
SELECT ab_job_num, sheet_theoretical_wt, ref_order_abc_num
INTO :ll_job_num, :ll_theo_wt, :ll_ref_order
FROM sheet_skid
...
ls_supplier_code = "033253-02"
ls_enduser_po = "ALCOAINC"
...
idw_requestor.Modify(ls_modstring)
ls_modstring = "bar_serial_t.Text = ~"*" + is_3 + Trim(String(al_skid_num)) + "*~""
idw_requestor.Modify(ls_modstring)
ls_modstring = "bar_serial_t_up.Text = ~"*" + is_3 + Trim(String(al_skid_num)) + "*~""
idw_requestor.Modify(ls_modstring)
...
//IF ib_theo_on THEN
IF 1 = 1 THEN	//always print theo wt
	IF ib_theo_kg THEN 
...
event ue_print_barcode_master;Datastore lds_print
Long ll_sheet_skid_num, ll_ab_job_num, ll_order_item_num, ll_order_abc_num
String ls_suplier_id, ls_modstring

lds_print = CREATE datastore  
lds_print.DataObject = "d_report_barcode_hayes_master"  
lds_print.SetTransObject (SQLCA)  
lds_print.Reset()
lds_print.Retrieve(al_shipment)

//retrieve supplier_id
SELECT sheet_skid_num
INTO :ll_sheet_skid_num 
FROM sheet_packing_item
WHERE packing_list = :al_shipment and sh_packing_item = 1
USING SQLCA;
...
SELECT order_item_num, order_abc_num
INTO :ll_order_item_num, :ll_order_abc_num 
FROM ab_job
...
INTO :ls_suplier_id 
FROM order_item
WHERE order_item_num = :ll_order_item_num and order_abc_num = :ll_order_abc_num
USING SQLCA;
 

ls_suplier_id = "033253-02"

//fill up the contents
ls_modstring = "supplier_id_t.Text = ~"" + ls_suplier_id + "~""	
lds_print.Modify(ls_modstring)

ls_modstring = "supplier_id_small_t.Text = ~"" + ls_suplier_id + "~""	
lds_print.Modify(ls_modstring)

ls_modstring = "bar_supplier_id_t_up.Text = ~"" + "*V" + ls_suplier_id + "*~""	
lds_print.Modify(ls_modstring)

ls_modstring = "bar_supplier_id_t.Text = ~"" + "*V" + ls_suplier_id + "*~""	
lds_print.Modify(ls_modstring)

ls_modstring = "bar_bol_t_up.Text = ~"" + "*2K" + String(al_shipment) + "*~""	
lds_print.Modify(ls_modstring)

ls_modstring = "bar_bol_t.Text = ~"" + "*2K" + String(al_shipment) + "*~""	
lds_print.Modify(ls_modstring)

ls_modstring = "bar_serial_t_up.Text = ~"" + "*5S" + String(al_shipment) + "*~""	
lds_print.Modify(ls_modstring)

ls_modstring = "bar_serial_t.Text = ~"" + "*5S" + String(al_shipment) + "*~""	
lds_print.Modify(ls_modstring)
...
on u_hayes_barcode_scale.create
call super::create
end on

on u_hayes_barcode_scale.destroy
call super::destroy
end on

event constructor;call super::constructor;is_objectname = "d_report_barcode_hayes"
li_allowsetup = 1
...
SELECT ab_job_num, sheet_theoretical_wt, ref_order_abc_num
INTO :ll_job_num, :lr_theo_wt, :ll_ref_order
FROM sheet_skid
...
IF li_count > 0 THEN
	...
	//print master barcode
	IF this.Event ue_print_barcode_master(al_shipment) <> 1 THEN RETURN -1
