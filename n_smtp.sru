...
Function long cryptSetAttributeString ( &
	...
	Ref string pBuff, &
	long StrLen &
	) Library "cl32.dll" Alias For "cryptSetAttributeString"
...
Function long cryptPopData ( &
	...
	) Library "cl32.dll" Alias For "cryptPopData"
...
Function long cryptPushData ( &
	...
	long StrLen, &
	Ref long pBytesCopied &
	) Library "cl32.dll" Alias For "cryptPushData"
...
Function long cryptGetAttributeString ( &
	...
	Ref string pBuff, &
	Ref integer StrLen &
	) Library "cl32.dll" Alias For "cryptGetAttributeString"
...
//Alex Gerlants. 08/22/2016. Begin
If ls_body = "ERROR TO GENERATE A GUID." Then
	of_Close(iul_socket)
	Return False
End If
//Alex Gerlants. 08/22/2016. End
...
// get a string from a pointer to a Unicode string and convert to Ansi
...
String ls_data, ls_subject, ls_word[]
Long ll_idx, ll_max
DateTime ldt_current
Blob lblob_encoded
...
If Len(ls_subject) > 80 Then
	...
	For ll_idx = 1 To ll_max
		If ls_word[ll_idx] <> "" Then
			...
			End If
			If ll_idx < ll_max Then
				lblob_encoded = Blob(ls_word[ll_idx] + " ")
			Else
				lblob_encoded = Blob(ls_word[ll_idx])
			End If
...
//MessageBox("of_databody for n_smtp", "ls_guid = " + ls_guid) //Alex Gerlants. 08/22/2016. TEST ONLY

If ls_guid = "ERROR TO GENERATE A GUID." Then Return ls_guid //Alex Gerlants. 08/22/2016
...
If UpperBound(istr_Attach) > 0 Then
	...
	For li_idx = 1 To li_max
		...
		If Lower(Left(ls_mimetype, 4)) = "text" Then
			...
			End If
			ls_data += CRLF + String(istr_Attach[li_idx].FileData) + CRLF
		Else
...
If lb_EncodeB Then
	...
	ls_base64 = of_Encode64(Blob(as_string))
...
If lb_EncodeB Then
	// Unicode - Encode 'base64'
	as_string = of_Encode64(Blob(as_string))
	as_encoding = "base64"
	as_charset  = "utf-8"
Else
	If lb_EncodeQ Then
...
lstr_UUID.Data4 = Blob(Space(8))
ls_Uuid = Space(SZ_UUID_LEN + 2)

//Alex Gerlants. 08/22/2016. Comment out begin. "Try-Catch" exception handling is not supported in machine code (.dll)
//try
...
//end try
//Alex Gerlants. 08/22/2016. Comment out end
...
If UuidCreateSequential(lstr_UUID) = RPC_S_OK Then
	If UuidToString(lstr_UUID, lul_ptrUuid) = RPC_S_OK Then
		...
		RpcStringFree(lul_ptrUuid)
	Else
		PopulateError(-1, "Error to generate a GUID.")
	End If
Else
	PopulateError(-1, "Error to generate a GUID.")
End If

//ls_Uuid = "Error to generate a GUID."  //TEST ONLY TEST ONLY TEST ONLY TEST ONLY TEST ONLY TEST ONLY TEST ONLY
//Alex Gerlants. 08/22/2016. End
