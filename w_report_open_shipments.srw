$PBExportHeader$w_report_open_shipments.srw
forward
global type w_report_open_shipments from w_sheet
end type
type st_2 from statictext within w_report_open_shipments
end type
type st_1 from statictext within w_report_open_shipments
end type
type cb_open from commandbutton within w_report_open_shipments
end type
type cb_get_size from commandbutton within w_report_open_shipments
end type
type st_customer from statictext within w_report_open_shipments
end type
type dw_customer_lookup from datawindow within w_report_open_shipments
end type
type cb_export from u_cb within w_report_open_shipments
end type
type dw_rebanded_list from datawindow within w_report_open_shipments
end type
type dw_rejcoil_list from datawindow within w_report_open_shipments
end type
type st_note from statictext within w_report_open_shipments
end type
type st_rowcount from statictext within w_report_open_shipments
end type
type cbx_all_dates from checkbox within w_report_open_shipments
end type
type cb_1 from commandbutton within w_report_open_shipments
end type
type st_all_dates from statictext within w_report_open_shipments
end type
type cb_retrieve from commandbutton within w_report_open_shipments
end type
type em_from from editmask within w_report_open_shipments
end type
type em_to from editmask within w_report_open_shipments
end type
type st_from from statictext within w_report_open_shipments
end type
type st_to from statictext within w_report_open_shipments
end type
type cbx_dateall from checkbox within w_report_open_shipments
end type
type dw_report from u_dw within w_report_open_shipments
end type
type cb_close from u_cb within w_report_open_shipments
end type
type cb_print from u_cb within w_report_open_shipments
end type
end forward
...
integer width = 4857
integer height = 2765
string title = "Open Shipments Report"
...
global w_report_open_shipments w_report_open_shipments
...
event type string ue_whoami();Return "w_report_open_shipments"
end event
...
public subroutine wf_resize ();This.Width = 5204
This.Height = 2509
This.X = 0
This.Y = 0

dw_report.Width = 5069
dw_report.Height = 1677
dw_report.X = 0
dw_report.Y = 400
end subroutine

on w_report_open_shipments.create
int iCurrent
call super::create
...
this.Control[iCurrent+4]=this.cb_get_size
this.Control[iCurrent+5]=this.st_customer
this.Control[iCurrent+6]=this.dw_customer_lookup
this.Control[iCurrent+7]=this.cb_export
this.Control[iCurrent+8]=this.dw_rebanded_list
this.Control[iCurrent+9]=this.dw_rejcoil_list
this.Control[iCurrent+10]=this.st_note
this.Control[iCurrent+11]=this.st_rowcount
this.Control[iCurrent+12]=this.cbx_all_dates
this.Control[iCurrent+13]=this.cb_1
this.Control[iCurrent+14]=this.st_all_dates
this.Control[iCurrent+15]=this.cb_retrieve
this.Control[iCurrent+16]=this.em_from
this.Control[iCurrent+17]=this.em_to
this.Control[iCurrent+18]=this.st_from
this.Control[iCurrent+19]=this.st_to
this.Control[iCurrent+20]=this.cbx_dateall
this.Control[iCurrent+21]=this.dw_report
this.Control[iCurrent+22]=this.cb_close
this.Control[iCurrent+23]=this.cb_print
end on

on w_report_open_shipments.destroy
call super::destroy
destroy(this.st_2)
destroy(this.st_1)
...
//ld_from_default = Date("01/01/1980") //From the beginning of time
...
event pfc_postopen;call super::pfc_postopen;wf_resize()
end event
...
If IsValid(This.inv_resize) THEN
   This.inv_resize.of_SetOrigSize(4831, 2768)

	This.inv_resize.of_Register(dw_report, "FixedToBottom")
	
	This.inv_resize.of_Register(cbx_all_dates, "FixedToBottom")
	This.inv_resize.of_Register(st_from, "FixedToBottom")
	This.inv_resize.of_Register(em_from, "FixedToBottom")
	This.inv_resize.of_Register(st_to, "FixedToBottom")
	This.inv_resize.of_Register(em_to, "FixedToBottom")
	This.inv_resize.of_Register(st_customer, "FixedToBottom")
	This.inv_resize.of_Register(dw_customer_lookup, "FixedToBottom")
	This.inv_resize.of_Register(st_note, "FixedToBottom")
	This.inv_resize.of_Register(cb_open, "FixedToBottom")
	This.inv_resize.of_Register(cb_get_size, "FixedToBottom")
	cb_get_size.Visible = False
	This.inv_resize.of_Register(st_1, "FixedToBottom")
	This.inv_resize.of_Register(st_2, "FixedToBottom")
...
string text = "Double-click on a row to open shipment"
...
gl_message = dw_report.GetItemNumber(ll_row, "packing_list" )
gl_customer = dw_report.GetItemNumber(ll_row, "customer_id" )

//Close(this) //Alex Gerlants. 05/20/2016

SetPointer(HourGlass!)
Message.StringParm = "w_shipping_2_disabled_buttons"
...
ls_file = "c:\temp\Open Shipments Report " + ls_date_from + "-" + ls_date_to + ".xls"
...
long textcolor = 33554432
long backcolor = 79741120
string text = "From the beginning of time to Present"
end type
...
//ls_sql_new = is_sql_orig + "  where shipment.shipment_status not in (0, 3) and (select count(*) from sheet_packing_item where packing_list = shipment.packing_list) > 0 "
ls_sql_new = is_sql_orig + "  where shipment.shipment_status not in (0, 3) and   ( (select count(*) from sheet_packing_item join sheet_skid on sheet_skid.sheet_skid_num = sheet_packing_item.sheet_skid_num where packing_list = shipment.packing_list) > 0 or (select count(*) from scrap_packing_item join scrap_skid on scrap_skid.scrap_skid_num = scrap_packing_item.scrap_skid_num where packing_list = shipment.packing_list) > 0 or (select count(*) from reject_coil_packing_item join reject_coil on reject_coil.coil_abc_num = reject_coil_packing_item.coil_abc_num join coil on coil.coil_abc_num = reject_coil_packing_item.coil_abc_num where packing_list = shipment.packing_list) > 0 ) "

ls_sql_new = ls_sql_new + ls_add_2sql_where

//ls_sql_new = ls_sql_new + " order by customer.customer_short_name"
...
integer height = 1494
integer taborder = 10
string dataobject = "d_report_open_shipments"
...
This.SetSort("customer_customer_short_name Asc")
This.Sort()
